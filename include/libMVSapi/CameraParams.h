...
typedef struct _MV_CC_DEVICE_INFO_
{
  ...
  unsigned int nTLayerType;  ///< \~chinese 设备传输层协议类型，e.g. MV_GIGE_DEVICE\~english Device Transport Layer
                             ///< Protocol Type, e.g. MV_GIGE_DEVICE
...
{
  int64_t nReviceDataSize;  ///< \~chinese 已接收数据大小  [统计StartGrabbing和StopGrabbing之间的数据量]\~english
                            ///< Received Data Size  [Calculate the Data Size between StartGrabbing and StopGrabbing]
...
/// \~chinese 图像结构体，输出图像指针地址及图像信息\~english Image Struct, output the pointer of Image and the
/// information of the specific image
...
typedef struct _MV_SAVE_IMAGE_PARAM_T_EX_
{
  ...
  unsigned int nBufferSize;  ///< [IN]    \~chinese 提供的输出缓冲区大小\~english Output buffer size provided
  enum MV_SAVE_IAMGE_TYPE enImageType;  ///< [IN]     \~chinese 输出图片格式\~english Output Image Format
  unsigned int nJpgQuality;  ///< [IN]    \~chinese JPG编码质量(50-99]，其它格式无效\~english Encoding
                             ///< quality(50-99]，Other formats are invalid
...
{
  unsigned int nType;  ///< \~chinese 需要输出的信息类型，e.g. MV_MATCH_TYPE_NET_DETECT\~english Information type need
                       ///< to output ,e.g. MV_MATCH_TYPE_NET_DETECT
...
/// \~chinese host收到从u3v设备端的总字节数，对应类型为 MV_MATCH_TYPE_USB_DETECT\~english The total number of bytes host
/// received from the u3v device side, the corresponding type is MV_MATCH_TYPE_USB_DETECT
...
/// \~chinese 可以从5模式下抢占权限，然后以独占权限打开\~english You can seize the authority from the 5 mode, and then
/// open with exclusive authority
...
/// \~chinese 可以从5的模式下抢占权限，然后以控制权限打开\~english You can seize the authority from the 5 mode, and then
/// open with control authority
...
/// \~chinese 可以从5的模式下抢占权限，然后以可被抢占的控制权限打开\~english You can seize the authority from the 5
/// mode, and then open with seized control authority
